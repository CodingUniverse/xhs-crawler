...
export function ContentClient({ initialContent }: ContentClientProps) {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm(showDeleted ? "确定要永久删除此项？" : "确定要删除此项？")) return
    try {
  ...
  const handleBatchDelete = async () => {
    if (!confirm(`确定要删除选中的 ${selectedItems.length} 项？`)) return
    try {
  ...
  const handleBatchRestore = async () => {
    if (!confirm(`确定要恢复选中的 ${selectedItems.length} 项？`)) return
    try {
  ...
  const handleQuickScrape = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errMsg = e instanceof Error ? e.message : "解析链接失败"
  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="flex flex-col lg:flex-row lg:justify-between lg:items-center mb-6 gap-4">
        <div>
          <h1 className="text-xl lg:text-2xl font-semibold text-[#1a1a1a] tracking-tight mb-1">内容资产</h1>
          <p className="text-[#737373] text-sm">查看和管理已采集的内容</p>
        </div>
      ...
      <div className="flex gap-4 mb-6 flex-wrap">
        ...
        >
          ...
          <option value="comments-desc">最多评论</option>
          <option value="shares-desc">最多分享</option>
      ...
      {selectedItems.length > 0 && (
        <div className="mb-4 flex items-center justify-between gap-2 flex-wrap bg-[#f0f9ff] border border-[#bae6fd] rounded-lg p-3">
      ...
      {loading ? (
        ...
        <div className="bg-white border border-[#e5e5e5] rounded-lg p-12 text-center shadow-sm">
          <p className="text-[#737373]">暂无内容，请先创建采集任务。</p>
        </div>
      ) : viewMode === "card" ? (
        <CardView
          items={content.items}
      ...
      <div className="flex justify-center items-center gap-2 mt-6">
        ...
        <span className="px-4 py-2 text-[#737373] text-sm">
          第 {content.page} 页，共 {Math.ceil(content.total / content.page_size)} 页
        </span>
        ...
        >
          ...
          <div className="p-3">
            <h3 className="font-medium text-sm line-clamp-2 mb-1 text-[#1a1a1a]">
              {item.title || item.content_text?.slice(0, 50) || "未命名"}
            </h3>
            <p className="text-xs text-[#737373] mb-2">
              {item.author_name || "未知"} · {platformLabels[item.platform]}
            </p>
      ...
      <table>
        <thead>
          <tr className="border-b border-[#e5e5e5] bg-[#fafafa]">
            <th className="text-left p-3 lg:p-4 font-medium text-[#525252] text-sm w-10">
            ...
            <th className="text-left p-3 lg:p-4 font-medium text-[#525252] text-sm w-20">封面</th>
            <th className="text-left p-3 lg:p-4 font-medium text-[#525252] text-sm min-w-[140px]">标题</th>
            <th className="text-left p-3 lg:p-4 font-medium text-[#525252] text-sm w-24">作者</th>
            <th className="text-left p-3 lg:p-4 font-medium text-[#525252] text-sm w-20">平台</th>
            <th className="text-left p-3 lg:p-4 font-medium text-[#525252] text-sm w-24 cursor-pointer hover:text-[#2563eb] whitespace-nowrap" onClick={() => onSort("publish_date")}>
              <span className="flex items-center gap-1">日期 <SortIcon field="publish_date" /></span>
            </th>
            <th className="text-left p-3 lg:p-4 font-medium text-[#525252] text-sm w-16 cursor-pointer hover:text-[#2563eb]" onClick={() => onSort("likes")}>
              <span className="flex items-center gap-1">点赞 <SortIcon field="likes" /></span>
            </th>
            <th className="text-left p-3 lg:p-4 font-medium text-[#525252] text-sm w-16 cursor-pointer hover:text-[#2563eb]" onClick={() => onSort("comments")}>
              <span className="flex items-center gap-1">评论 <SortIcon field="comments" /></span>
            </th>
        ...
        <tbody>
          {items.map((item) => {
            ...
            <tr key={item.id} className={`border-b border-[#e5e5e5] last:border-0 hover:bg-[#fafafa] ${isSelected ? "bg-[#f0f9ff]" : ""}`}>
              ...
              <td className="p-3 lg:p-4">
                <div className="w-10 h-10 lg:w-12 lg:h-12 bg-[#f5f5f5] rounded overflow-hidden">
                  {validImages[0] ? (
                    <img
                      src={validImages[0]}
                      alt=""
              ...
              <td className="p-3 lg:p-4 text-[#1a1a1a] text-sm truncate max-w-[200px]">{item.title || item.content_text?.slice(0, 30) || "-"}</td>
              <td className="p-3 lg:p-4 text-[#1a1a1a] text-sm truncate max-w-[120px]">{item.author_name || "-"}</td>
              <td className="p-3 lg:p-4">
                <span className="px-2 py-1 bg-[#eff6ff] text-[#2563eb] rounded text-xs font-medium whitespace-nowrap">
                  {platformLabels[item.platform]}
                </span>
              </td>
              <td className="p-3 lg:p-4 text-sm text-[#737373] whitespace-nowrap">
              ...
              <td className="p-3 lg:p-4 text-[#1a1a1a] text-sm">{item.metrics?.likes || 0}</td>
              <td className="p-3 lg:p-4 text-[#1a1a1a] text-sm">{item.metrics?.comments || 0}</td>
              <td className="p-3 lg:p-4">
...
function WorkspaceModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="relative bg-white border border-[#e5e5e5] rounded-lg w-full max-w-[1400px] mx-4 h-[90vh] flex flex-col shadow-xl">
        <button
          onClick={onClose}
        ...
        <div className="flex flex-col lg:flex-row flex-1 overflow-hidden">
          <div className="w-full lg:w-1/2 lg:border-r border-b lg:border-b-0 border-[#e5e5e5] overflow-y-auto">
            <div className="sticky top-0 bg-white z-10 p-4 border-b border-[#e5e5e5]">
              <div className="flex items-center gap-2">
                <span className="px-2 py-1 bg-[#eff6ff] text-[#2563eb] rounded text-xs font-medium whitespace-nowrap">
                  {platformLabels[item.platform]}
                </span>
                <h2 className="text-lg font-semibold text-[#1a1a1a] line-clamp-1">{item.title || "未命名"}</h2>
              </div>
              <p className="text-sm text-[#737373] mt-1 flex items-center flex-wrap gap-x-1">
                <span suppressHydrationWarning>
                  {item.author_name} · {item.publish_date ? new Date(item.publish_date).toLocaleDateString("zh-CN") : "未知日期"}
                </span>
                {item.source_url && (
                  <a
              ...
              <div className="relative">
                <div className="aspect-video bg-[#1a1a1a] flex items-center justify-center">
                  {images[currentImageIndex] ? (
                    ...
                    <span className="text-[#737373]">无图片</span>
                  )}
                </div>
                {images.length > 1 && (
                  <>
            ...
            <div className="p-4">
              <div className="flex items-center gap-4 mb-4 text-sm border-b border-[#e5e5e5] pb-4 flex-wrap">
                <span className="flex items-center gap-1.5 text-[#525252]">
                  <Heart className="w-4 h-4" /> {item.metrics?.likes || 0} 点赞
                </span>
                <span className="flex items-center gap-1.5 text-[#525252]">
                  <MessageCircle className="w-4 h-4" /> {item.metrics?.comments || 0} 评论
                </span>
                <span className="flex items-center gap-1.5 text-[#525252]">
                  <Share2 className="w-4 h-4" /> {item.metrics?.shares || 0} 分享
                </span>
                <span className="flex items-center gap-1.5 text-[#525252]">
                  <Eye className="w-4 h-4" /> {item.metrics?.views || 0} 浏览
                </span>
              </div>

              <h3 className="font-medium text-[#1a1a1a] mb-2">正文内容</h3>
              <p className="text-[#525252] whitespace-pre-wrap text-sm leading-relaxed">
                {item.content_text || "暂无正文内容"}
              </p>
            </div>
          </div>
          ...
          <div className="w-full lg:w-1/2 flex flex-col">
            <div className="p-4 border-b border-[#e5e5e5] flex items-center justify-between">
              <h3 className="font-semibold text-[#1a1a1a]">工作区</h3>
              <button
            ...
            {aiResult && (
              <div className="p-4 border-b border-[#e5e5e5] bg-[#faf5ff]">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="font-medium text-sm text-[#1a1a1a]">AI 分析结果（模拟）</span>
                </div>
                <div className="space-y-2 text-sm">
                  <p className="text-[#525252]"><strong>钩子：</strong>{(aiResult as { hook: string }).hook}</p>
                  <div>
                    <strong className="text-[#1a1a1a]">结构：</strong>
                  ...
                  <p className="text-[#525252]"><strong>热门词：</strong>{((aiResult as { hot_phrases: string[] }).hot_phrases || []).join("、")}</p>
                  <p className="text-[#525252]"><strong>引导语：</strong>{(aiResult as { call_to_action: string }).call_to_action}</p>
                </div>
              </div>
            )}
            ...
            <div className="flex-1 p-4 flex flex-col">
              <div className="flex items-center justify-between mb-2">
                <label className="font-medium text-[#1a1a1a]">笔记 / 大纲</label>
                {saving && <span className="text-xs text-[#737373] flex items-center gap-1"><Loader2 className="w-3 h-3 animate-spin" /> 保存中...</span>}
              </div>
              <textarea
                value={outline}
                onChange={(e) => handleOutlineChange(e.target.value)}
                placeholder="在此撰写大纲、笔记或改写思路..."
                className="flex-1 w-full p-4 bg-[#fafafa] border border-[#e5e5e5] rounded-lg resize-none text-sm text-[#1a1a1a] focus:outline-none focus:ring-2 focus:ring-[#2563eb] focus:border-transparent leading-relaxed"
              />
              <button
                onClick={async () => {
                  setSaving(true)
